package com.unclecole.brbarriers.cmd;

import com.unclecole.brbarriers.cmd.subs.*;
...
public class BaseCommand implements CommandExecutor {
    ...
    public BaseCommand() {
        ...
        addCommand(new BarriersCmd());
        addCommand(new GiveKeyCmd());
        addCommand(new CreateCmd());
        addCommand(new ListCmd());
        addCommand(new DeleteCmd());
        addCommand(new ViewCmd());
        addCommand(new CreateKeyCmd());
        addCommand(new ListKeysCmd());
        addCommand(new DeleteKeyCmd());
    }

    public boolean onCommand(CommandSender s, Command cmd, String string, String[] args) {
        try {
            if (args.length == 0) {
                AbstractCommand helpCommand = this.subcommands.get(Class.forName("com.unclecole.brbarriers.cmd.subs.BarriersCmd"));

                if (s.hasPermission(helpCommand.getPermission())) {
                    (this.subcommands.get(Class.forName("com.unclecole.brbarriers.cmd.subs.BarriersCmd"))).execute(s, args);
                    return false;
                }
